...
def load(
  ...
  Args:
    ...
    name: Which dataset to use, (must be '')
  ...
  def fit_generator_aug(ds):
    for data in ds:
        x = tf.reshape(data['images'], (-1, 224, 224, 3))
        y = augment_labels(data)
  ...
  def augment_labels(data):
    """
      If cutmix, mixup or cutup augmentation is used, then the labels are 
      altered too.
      1. The labels are onehot encoded.
      2. The ratio of augmentation done on the images is applied to the
        label of that image. 
      3. The original label and mixed label are added together in the proportion given in mix_labels.
      4. The extra dimension of output is removed { (1, {batch_size}, 1, {num_classes}) -> (1, {batch_size}, {num_classes}) }
    """

    y = tf.reshape(data['labels'], (-1,))    
    y = tf.one_hot(y, 15)

    if "mix_labels" in data:

      y1 = tf.reshape(data['mix_labels'], (-1,))    
      y1 = tf.one_hot(y1, 15)
      ratio = tf.reshape(data["ratio"], (-1,1))
      y = ratio * y + (1 - ratio) * y1
    # y = tf.reshape(y, (-1,))    

    return y

  
  # COMPLETED-> TODO replace jax.process_index() with tensorflow equivalent 
  # start, end = _shard(split, jax.process_index(), jax.process_count()) 
  
  # replace jax.process_index() with tensorflow equivalent 
  replica_context = tf.distribute.get_replica_context()
  start, end = _shard(split, 
  ...
  if name == 'leaf_classification':
    tfds_split = tfds.core.ReadInstruction(split,
                                           from_=start, to=end, unit='abs')
    print(f'Loading {name} from {data_dir}')
    ds = tfds.load(f'{name}:5.0.0', split=split.split,
                   decoders={'image': tfds.decode.SkipDecoding()},
                   data_dir=data_dir, download=False)

    
  else:
    raise ValueError('Only leaf_classification:5.0.0 is presently supported for this dataset.')
  ...
  if is_training:
    # if jax.process_count() > 1: # TODO change to tf equivalent
#     if replica_context.num_replicas_in_sync > 1:
...
#       ds = ds.cache()
    ...
    if split.num_examples % total_batch_size != 0:
      ds = ds.skip(split.num_examples % total_batch_size) # TODO somehow don't drop images in validation and test sets
#       raise ValueError(f'Test: {split.num_examples} must be divisible by {total_batch_size}')
  ...
  def augment_normalize(batch):
    ...
    # TO DO: Complete code to convert label size
    #batch['labels'] = augment_labels(batch)
  ...
  for i, batch_size in enumerate(reversed(batch_dims)):
    if i == 0:
      ...
      if augment_name is not None:
        ...
        if 'cutout' in augment_name:
          logging.info('Applying Cutout!')
          ds = ds.map(my_cutout, num_parallel_calls=AUTOTUNE)
  ...
  # Added this part so as to integrate the augmentation
  # in the data generator itself.
  ds = fit_generator_aug(ds)
...
def my_cutout(batch):
    """Cutout. with cutout factor=0.5"""
    batch = dict(**batch)
    imgs = batch['images']
    bs = tf.shape(batch['images'])[0]
    labels = batch['labels']
    h, w = 84, 84 # TODO HARD CODED, change later
    fill_values = tf.random.uniform(shape=[bs, 1, 1, 3])
    aug_imgs = tfa.image.random_cutout(imgs, 
                             (h, w),
                             constant_values = fill_values)

    return {"images": aug_imgs, "labels": labels}
